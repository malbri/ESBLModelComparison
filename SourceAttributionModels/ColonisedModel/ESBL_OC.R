...
human_data <- Genes_data[Genes_data$Source == "Human_OC", ]
Updated_data <- Genes_data[Genes_data$Source != "Human_OC", ]
...
# Find prevalence of Gene i in source j
...
nsim = 100000
nthin = 10
parms = c("a","q", "lambdai", "lambdaj","lambdaij") #,"lambdai", "lambdaj"


# Starting the model with a chain and 1000 iterations for adaptation
m <- jags.model("ESBL_OC.txt", jagsdata, inits, n.chains=3,n.adapt=1000)
...
lambdaij2=matrix(lambdaij1, ncol=11, nrow=20,
                 dimnames = list(bd.obs$Genes,
                                 c("Broilers", "Broilers_IMP", "Cat", "Cattle", "Cattle_IMP", "Dog","Horse", "Human_CL", "Pig", "Pig_IMP", "Turkey_IMP")))
summary(lambdaij2)
...
pred.obs <- pred.obs %>%
  mutate(Not_predicted = Count - (Broilers + Broilers_IMP + Cat + Cattle + Cattle_IMP + Dog + Horse + Human_CL + Pig + Pig_IMP + Turkey_IMP)) %>%
...
results_percentage$Source <- factor(results_percentage$Source,
                                    levels = c("Broilers", "Cattle", "Pig",  # Food group
                                               ...
                                               "Human_CL",                 
                                               "Broilers_IMP", "Cattle_IMP", "Turkey_IMP", "Pig_IMP",# Import group
                                               "Not_predicted")  
)
...
#Plot
SA_OC_gene_specific <- ggplot(results_percentage, aes(x = Gene, y = Percentage, fill = Source)) +
...
gene_sorting <- results_percentage %>%
  ...
  mutate(Sort_Order = case_when(
    ...
    Import_pct > 60 ~ 2,   # Other Import > 40%
    Human_pct > 40 ~ 3,    # Human > 40%
...
SA_OC_gene_specific_sorted <- ggplot(results_percentage, aes(x = Gene, y = Percentage, fill = Source)) +
  geom_bar(stat = "identity") +
  scale_y_continuous(
    breaks = c(0, 20, 40, 60, 80, 100),  # Define breaks for the y-axis
    labels = custom_y_labels
...
print(SA_OC_gene_specific_sorted)

#save
#ggsave("C:/Users/ColonisedModel/SA_OC_gene_specific.png", plot = SA_OC_gene_specific_sorted)

write_xlsx(pred.obs, "obs_pred_counts_OC.xlsx")
...
custom_labels <- c("Broiler", "Broiler Import", "Cat", "Cattle", "Cattle Import", "Dog", "Horse", "Human Clinical", "Pig", "Pig Import", "Turkey Import")

SA_OC_output <- ggplot(output, aes(x = Source, y = Mean)) +
...
#ggsave("C:/Users/ColonisedModel/SA_OC_output.png", plot = SA_OC_output)
